...
static const bool ENABLE_ESPNOW = true;

// Proof-of-concept MAC from a tested setup.
// Replace with YOUR receiver ESP32 MAC address if ENABLE_ESPNOW = true.
static uint8_t GROUND_STATION_MAC[6] = {0xC8, 0xF0, 0x9E, 0xF4, 0x68, 0x8C};
...
String buildCompactLinkJson(const String &fullJson, uint32_t nowMs) {
  ...
  float theta = phase * 2.0f * PI;
  float lat = 51.6f * sinf(theta);
  float lon = wrapDeg((phase * 360.0f * 15.0f) - 180.0f);
  bool eclipse = (phase > 0.31f && phase < 0.68f);

  float alt = 525.0f + 2.8f * sinf(theta * 0.2f);
  float soc = 76.0f + 10.0f * sinf(theta - 0.7f);
  if (eclipse) soc -= 6.0f;
  soc = clampf(soc, 18.0f, 99.0f);
  float tb = 18.0f + (eclipse ? -1.0f : 1.0f) * 7.5f + 1.2f * sinf(theta * 1.3f);
  float snr = 9.5f + 3.0f * sinf(theta * 1.1f);
  int rssi = (int)(-111.0f + 7.0f * sinf(theta * 0.9f));

  char compact[240];
  snprintf(compact,
           sizeof(compact),
           "{\"t\":\"hb\",\"node\":\"%s\",\"mt\":%.0f,\"lat\":%.2f,\"lon\":%.2f,\"e\":%s,\"soc\":%.2f,\"tb\":%.2f,\"alt\":%.2f,\"snr\":%.2f,\"rssi\":%d,\"src\":\"espnow\"}",
           SIM_NAME,
           tSec,
           lat,
           lon,
